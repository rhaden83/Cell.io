class Creature extends SoftBody {
  ...
  public void drawBrain(PFont font, float scaleUp, int mX, int mY) {
    ...
    fill(0, 0, 1);
    String[] inputLabels = { "0Hue", "0Sat", "0Bri", "1Hue", "1Sat", "1Bri", "2Hue", "2Sat", "2Bri", "Size", "Mem", "Const." };
    String[] outputLabels = { "BHue", "MHue", "Accel.", "Turn", "Eat", "Fight", "Birth", "How funny?", "How popular?", "How generous?", "Mem", "Const." };
  ...
  public void drawSoftBody(float scaleUp, float camZoom, boolean showVision) {
    ...
    if(showVision) {
      for(int i = 0; i < visionAngles.length; i++) {
        ...
        line((float)((visionOccludedX[i] - CROSS_SIZE) * scaleUp), (float)((visionOccludedY[i] - CROSS_SIZE) * scaleUp), (float)((visionOccludedX[i] + CROSS_SIZE) * scaleUp),
    ...
    if(this == board.selectedCreature) {
      ellipse((float)(px * scaleUp), (float)(py * scaleUp), (float)(radius * scaleUp + 1 + 75.0 / camZoom), (float)(radius * scaleUp + 1 + 75.0 / camZoom));
    }
    ...
    for(int k = 0; k < visionAngles.length; k++) {
      ...
      for(int i = 0; i < potentialVisionOccluders.size(); i++) {
        ...
        if(Math.abs(translatedY) <= r) {
          if((translatedX >= 0 && translatedX < visionLineLength && translatedY < visionLineLength) || distance(0, 0, translatedX, translatedY) < r
